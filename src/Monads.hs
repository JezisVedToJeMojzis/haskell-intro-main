...
--addTwo = add x 2
--multiply = mul x addTwo
bogus x = do
   addTwo <- add x 2  -- Add 2 to x
   res <- mul x addTwo  -- Multiply x by (x + 2)
   return res  -- Return the result wrapped in Identity
...
oneEighth x = do
   oneHalf <- half x
   oneQuarter <- half oneHalf
   oneEighth <- half oneQuarter
   return oneEighth
